...
try {
    $stmt = $pdo->query('SELECT id_pais, pais FROM Pais ORDER BY pais');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($dni === '' || $nombre === '' || $apellido === '' || $correo === '' || $credencial === '' || $credencial_confirmacion === '' || $pais_seleccionado === '') {
    ...
    } elseif ($credencial !== $credencial_confirmacion) {
        ...
        try {
            ...
            $stmt->execute([
                ':correo' => $correo,
                ':credencial' => password_hash($credencial, PASSWORD_ARGON2ID)
            ]);

            $id_cuenta = $pdo->lastInsertId();
            $stmt = $pdo->prepare('INSERT INTO Huesped (dni_huesped, nombre_huesped, apellido_huesped, id_cuenta, id_pais) VALUES (:dni, :nombre, :apellido, :id_cuenta, :id_pais)');
            $stmt->execute([
                ':dni' => $dni,
                ':nombre' => $nombre,
                ':apellido' => $apellido,
...
<html lang="es">
    ...
    <body class="bg-gray-50 min-h-screen flex flex-col m-0">
        ...
        <main class="flex-grow flex items-center justify-center px-4 py-10">
            <section class="bg-white p-8 rounded-xl shadow-md border border-gray-100 max-w-lg w-full">
                <h1 class="text-2xl font-bold text-gray-800 text-center">Registrar Huésped</h1>

                <?php if (isset($_GET['registro'])): ?>
                    <p class="mt-4 bg-green-100 text-green-700 p-3 rounded-lg text-center">Huésped registrado correctamente.</p>
                <?php endif; ?>
                <?php if ($error !== ''): ?>
                    <p class="mt-4 bg-red-100 text-red-700 p-3 rounded-lg text-center"><?php echo htmlspecialchars($error); ?></p>
                <?php endif; ?>
